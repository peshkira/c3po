...
public class Element {
	...
	private static final String[] PATTERNS = { "yyyy:MM:dd HH:mm:ss",
			"MM/dd/yyyy HH:mm:ss", "dd MMM yyyy HH:mm",
			"EEE dd MMM yyyy HH:mm", "EEE, MMM dd, yyyy hh:mm:ss a",
			"EEE, MMM dd, yyyy hh:mm a", "EEE dd MMM yyyy HH.mm",
	...
	/**
	 * Tries to parse a creation date out of the name of the current element.
	 * Some sources include a timestamp in the name. The usage of this method
	 * can be configured through the adaptor config parameter
	 * {@link Constants#CNF_INFER_DATE}
	 * 
	 * If the name is not set or it does not include a timestamp, the method
	 * does nothing.
	 * 
	 * @param created
		...
		if (this.name != null) {
			...
			if (split.length > 2) {
				...
				try {
					...
					// if the value is not a number then it is something else
					// and not a
		...
		for (MetadataRecord r : this.getMetadata()) {
			...
			if (r.getStatus().equals(Status.CONFLICT.name())) {
				...
				if (meta.containsField(r.getProperty().getId())) {
					conflicting = (BasicDBObject) meta.get(r.getProperty()
							.getId());
					...
					values.add(this.getTypedValue(r.getProperty().getType(),
							r.getValue()));

					// TODO: this is evil... just reducing information at
					// random... Possibly a map would be the better way to map
					// values to their sources
				...
				key.put("value",
						this.getTypedValue(r.getProperty().getType(),
								r.getValue()));
	 ...
	 * converts the value. Otherwise it leaves the string representation. This
	 * is valuable as the underlying persistence layer can store the native type
	...
	/**
	 * Removes all records for the given property id and returs a list of all
	 * removed metadata records.
	 * 
	 * @param property
	 *            the id of the property
	 * @return returns the records of the element matching this property that
	 *         were deleted.
	 */
	...
	/**
	 * A boolean representation of the passed string. If the string equals one
	 * of 'yes', 'true' or 'no', 'false' then the value is converted to the
	 * corresponding boolean. Otherwise null is returned
	 * 
	 * @param value
	 *            the value to convert
	 * @return the boolean representation of the value, or null if not a
	 *         boolean.
	 */
	...
	private Date getDateValue(String value) {
		...
		if (result == null) {
			LOG.debug("No pattern matching for value {}, try to parse as long",
					value);
		}
		...
		try {
			...
			LOG.trace(
					"date is not in long representation, trying pattern matching: {}",
					e.getMessage());
		}
