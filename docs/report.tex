...
\begin{enumerate}
 \item \textbf{Update resolved conflicts}: If all metadata records for a given property are marked as conflicting, but all values are equal, merge them to a single record and update the status to ''SINGLE_VALUE``. This rule has the highest priority of 999, so it is always applied first after an element got changed.
